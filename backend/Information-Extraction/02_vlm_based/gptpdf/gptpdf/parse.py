...
def _parse_pdf_to_images(pdf_path, output_dir = './'):
    ...
    for page_index, page in enumerate(pdf_document):
        ...
        # 保存原始页面图片（在绘制任何标注之前）
        original_page_image = os.path.join(output_dir, f'{page_index}_original.png')
        original_pix = page.get_pixmap(matrix=fitz.Matrix(3, 3))
        original_pix.save(original_page_image)
